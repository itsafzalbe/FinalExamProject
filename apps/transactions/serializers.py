...
class CategoryCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_parent_category(self, value):
        """Ensure parent category belongs to same user or is default"""
        if value:
            request = self.context.get('request')
            if value.user and value.user != request.user:
                raise serializers.ValidationError(
                    "Cannot use another user's category as parent"
                )
        return value


class TransactionTagSerializer(serializers.ModelSerializer):
    """Serializer for transaction tags"""
    is_default = serializers.ReadOnlyField()
...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Transaction
        fields = [
            'id', 'type', 'title', 'amount', 'date',
            'card', 'card_name', 'card_currency',
            'category', 'category_name', 'category_icon', 'category_color',
            'amount_in_user_currency', 'tags', 'created_at'
        ]
    ...
    def get_tags(self, obj):
        ...
        return TransactionTagSerializer(
            [relation.tag for relation in tag_relations],
            many=True
        ).data
...
class TransactionDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'card', 'card_name', 'card_currency', 'card_type',
            'category', 'category_name', 'category_icon', 'category_color',
            'amount_in_user_currency', 'exchange_rate_used',
            'receipt_image', 'location', 'tags',
...
class TransactionCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Transaction
        fields = [
            'card', 'category', 'type', 'amount', 'title',
            'description', 'date', 'receipt_image', 'location'
        ]
    ...
    def validate_card(self, value):
        ...
        if value.user != request.user:
            raise serializers.ValidationError("You can only create transactions for your own cards")
        return value
    ...
    def validate(self, data):
        ...
        # Check if card has sufficient balance for expenses
        if data['type'] == 'expense':
            card = data['card']
            if not card.can_withdraw(data['amount']):
                raise serializers.ValidationError({
                    'amount': f"Insufficient balance. Card has {card.balance} {card.currency.code}"
                })
